...
public class MorseProcessor {
    ...
    public MorseProcessor(String filename) throws WavFileException, IOException {
        ...
        this.sampleRate = ((int) wavFile.getSampleRate() / 1000) * checkStateInterval; // 44 frames per ms * 10 = 10ms sample length
    ...
    public List<MorseSignal> getSignals() throws WavFileException, IOException {
        List<MorseSignal> signals = new ArrayList<>();
        ...
        while (this.wavFile.getFramesRemaining() > 0) // test if we have reached the end of the file
        {
            ...
            if (!first_silence_flag && next_signal.isSilence && signals.isEmpty()) {
    ...
    public MorseSignal readNextSignal() throws WavFileException, IOException {
        ...
        if (sample > minSampleValue) { // if the sample value is greater than 0.01 it is a valid signal
        ...
        while (this.wavFile.getFramesRemaining() > 0) {
            ...
            if ((signal.isSilence && tmp_sample > minSampleValue) // if we got a silence and now a signal
                    || (signal.isSignal && tmp_sample <= minSampleValue)) // if we got a signal and now a silence
            {
    ...
    private float readSample() throws WavFileException, IOException {
        if (this.wavFile.getFramesRemaining() > 0) {
            ...
            List<Float> positiveSamples = new ArrayList<>(); // we want only positive value
    ...
    private List<String> finalize(List<MorseSignal> signals) {
        ...
        while (first_iterator.hasNext()) {
            ...
            if (!signal.isSilence && !this.seek(signals_length, (signal.length * checkStateInterval))) {

                signals_length.add(signal.length * checkStateInterval);
                signal_total_value += signal.length * checkStateInterval;

            } else if (signal.isSilence && !this.seek(signals_length, (signal.length * checkStateInterval))) {

                silences_length.add(signal.length * checkStateInterval);
                silence_total_value += signal.length * checkStateInterval;
            }
        ...
        while (second_iterator.hasNext()) {
            ...
            if (signal.isSignal) {
                ...
                // check if silence is a "long" silence between two words
